from fastapi import APIRouter, File, HTTPException, UploadFile, Query, Form
...
from app.services.storage_service import save_report, get_report
...
async def analyze_report(
    ...
    try:
        ...
        extraction = analyze_lab_report_images(images_bytes, target_language=target_language)
        
        # 3. Return structured JSON as dict to avoid FastAPI response validation 500s
        extraction_dict = extraction.model_dump() if hasattr(extraction, 'model_dump') else dict(extraction)
